...
- Start with `accounting/CURRENT_AUTHORITY_INDEX.md`, `accounting/PROJECT_INSTRUCTIONS.md`, `accounting/FASB_APPLICABILITY_INDEX.md`, `accounting/ACCOUNTING_POLICY_MATRIX.md`, and `accounting/manifests/fasb_topic_registry.json`. For tax work, also read `accounting/TAX_PROFILE_ASSUMPTIONS.md`, `accounting/TAX_OPPORTUNITY_AND_COMPLIANCE_MATRIX.md`, and `accounting/manifests/tax_authority_registry.json`.
...
- Run `python accounting/scripts/check_tax_sources.py --validate-only` after changing the tax registry, tax profile, tax decision matrix, source cross-references, or tax-monitor workflow.
- Keep taxpayer identifiers, names of spouses/employers, addresses, wage/withholding amounts, returns, and private workpapers out of GitHub; store only sanitized routing facts and controls.
- Do not auto-merge an accounting- or tax-authority currentness or policy change.
