...
- For Sign Date, use only `{{SD:R<n>:(dateformat="MM/dd/yyyy hh:mm a z")}}`, substituting exactly one explicit R1-R25 role per tag. Do not add `*`; Zoho documents that required marker for text fields and checkboxes, while Sign Date is system-populated after signing. A combined-recipient tag is invalid even if Zoho forms a field, because controlled testing assigned that field only to the first parsed recipient. Every Zoho Sign text tag must fit completely—from its opening braces through both closing braces—on one rendered physical line and one page. Automatic visual wrapping is a failure even when the source contains no manual newline; widen or merge the field area, reduce only the source-tag font, or restructure the layout before sending. Tables use the same syntax and have no special tag grammar. Keep noncanonical Sign Date formats, multi-recipient tags, and choice-field tags blocked unless separately approved through the governed smoke-test process.
