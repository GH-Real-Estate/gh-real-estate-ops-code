...
- [ ] Confirm a consolidated interest invoice uses `true` if any governed source allows partial payment, `false` only if every known source disables it, and omits the field if all source values are absent.
