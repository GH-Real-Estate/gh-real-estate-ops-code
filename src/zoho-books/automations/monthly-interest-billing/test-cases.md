...
| RF / NSF invoice overdue | Source invoice/header/line indicates RF, NSF, returned payment, or returned check | Excluded from interest |
...
4. Re-run for the same billing month and confirm no duplicate invoice is created.
5. Inspect the created invoice line descriptions: they must include source invoice number, APR, interest period, and days.
6. Inspect each source rent invoice: it must receive the interest period and invoice ID tokens.
