...
| Partial payment | One source rent invoice has `allow_partial_payments=true` and total exceeds current balance | Interest uses current outstanding balance; the source setting remains enabled and the created interest invoice carries `true` |
| Mixed partial-payment settings | Consolidated customer rows include at least one `true` and one `false` | Created interest invoice uses `true`; source invoices remain unchanged |
| All known settings disabled | Every non-null source value is `false` | Created interest invoice uses `false`; source invoices remain unchanged |
| Partial-payment setting absent | Every source response omits the field | Created interest payload omits `allow_partial_payments`; source invoices remain unchanged |
...
9. Confirm the created interest invoice uses `true` if any governed source allows partial payment, `false` only if every known source disables it, and omits the field if all source values are absent.
