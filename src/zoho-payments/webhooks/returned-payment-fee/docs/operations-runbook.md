...
Do not hardcode the live endpoint in this repository.

Store the current endpoint in the approved runtime/deployment system and in your private operations notes. If it must be referenced here, use a sanitized placeholder:

```text
https://<catalyst-app>.catalystserverless.com/server/<function-name>/zpayments/payment-failed
```

Use the actual endpoint in Zoho Payments for the `payment.failed` webhook only after signature verification and replay protection are configured.
...
```text
A verified failed payment creates a returned-payment fee invoice and sends it if sending is enabled.
```
...
After testing, return to live mode only when ready:
...
```text
Zoho Payments webhook URL points to the approved Production endpoint
...
Returned fee amount matches the final lease
DRY_RUN=false
SEND_INVOICE_ON_CREATE=true
...
Payment attempt token is recorded
...
```text
Inspect the Zoho Payments payment description and metadata in Zoho directly. Do not copy raw production data into GitHub.
```
...
```text
Confirm ZOHO_CLIENT_ID, ZOHO_CLIENT_SECRET, and ZOHO_REFRESH_TOKEN exist in the active runtime environment.
```
...
If manual replay is needed, always use a unique sanitized test `event_id` or process through the approved operational route.
...
5. Record a sanitized summary in docs/runbooks/deployment-log.md.
```
