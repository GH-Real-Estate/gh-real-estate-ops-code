...
For Sign Date, run an ordinary-paragraph control and the ranked table-cell tests in the generated Sign registry. Begin with the unchanged `{{SD:R1:(dateformat="MM/dd/yyyy hh:mm a z")}}` standard in a wide or merged Date cell. Controlled testing confirmed that valid `hh` tags remained literal when visually wrapped, while fields formed in the same table when their complete source tags fit. Treat any automatic visual wrap as failure even if no manual newline exists, and apply that gate to every Zoho Sign text tag. Fail the test if any portion of a tag—including either closing brace—crosses a rendered line, row, or page boundary, remains literal, or is assigned to the wrong recipient. Disable **Allow row to break across pages** for every signature row. The 2026-07-17 combined-recipient probes are negative evidence: some formed a field, but only the first recipient owned it, so every intended signer must have a separate tag and field.
