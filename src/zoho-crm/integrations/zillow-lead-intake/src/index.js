...
function buildConfig() {
  ...
  return {
    version: env('GATEWAY_VERSION', '2026-07-07.v6'),
...
function handleHealth(req, res, requestId) {
  ...
  if (!safeTimingEqualText(getHeader(req.headers, CONFIG.healthHeaderName), CONFIG.healthToken)) {
...
function normalizeZillowLeadPayload(payload, receivedAtIso = new Date().toISOString(), sourcePayloadHash = '') {
  ...
  const secondaryPhone = normalizePhone(firstValue(input, [
    'secondary_phone', 'secondaryphone', 'alternate_phone', 'alternatephone', 'other_phone',
    'phone_2', 'phone2', 'home_phone', 'work_phone', 'secondary_mobile'
  ]));
...
function normalizeInputObject(input) {
  const output = {};
  for (const [key, value] of Object.entries(input || {})) output[normalizeFieldKey(key)] = Array.isArray(value) ? value.find((item) => cleanText(item)) : value;
  return output;
}
...
function normalizeDate(value) {
  ...
  const yyyymmdd = text.match(/^(\d{4})(\d{2})(\d{2})$/); if (yyyymmdd) return `${yyyymmdd[1]}-${yyyymmdd[2]}-${yyyymmdd[3]}`;
  const isoMatch = text.match(/^(\d{4})-(\d{2})-(\d{2})/); if (isoMatch) return `${isoMatch[1]}-${isoMatch[2]}-${isoMatch[3]}`;
  const usMatch = text.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/); if (usMatch) return `${usMatch[3]}-${String(usMatch[1]).padStart(2, '0')}-${String(usMatch[2]).padStart(2, '0')}`;
  const parsed = new Date(text); return Number.isNaN(parsed.getTime()) ? '' : isoDateOnly(parsed.toISOString());
}
...
function buildRenterProfileSummary(profile) {
  return [
    ['Bedrooms sought', profile.numBedroomsSought], ['Bathrooms sought', profile.numBathroomsSought], ['Neighborhoods', formatPossiblyJson(profile.neighborhoods)],
    ['Property types desired', formatPossiblyJson(profile.propertyTypesDesired)], ['Lease length months', profile.leaseLengthMonths], ['Smoker', profile.smoker],
    ['Parking desired', profile.parkingTypeDesired], ['Income yearly', profile.incomeYearly === undefined ? '' : String(profile.incomeYearly)],
    ['Credit score range', formatPossiblyJson(profile.creditScoreRangeJson)], ['Moving from city', profile.movingFromCity], ['Moving from state', profile.movingFromState],
    ['Move-in timeframe', profile.moveInTimeframe], ['Reason for moving', profile.reasonForMoving], ['Employment status', profile.employmentStatus],
    ['Job title', profile.jobTitle], ['Employer', profile.employer], ['Employment start date', profile.employmentStartDate],
    ['Employment details', formatPossiblyJson(profile.employmentDetailsJson)], ['Pet details', formatPossiblyJson(profile.petDetailsJson)]
  ].filter(([, value]) => cleanText(value)).map(([label, value]) => `${label}: ${truncate(value, 700)}`).join('\n');
}
...
function readRequestBody(req, maxBytes, timeoutMs) {
  if (Buffer.isBuffer(req.rawBody)) return req.rawBody.length > maxBytes ? Promise.reject(publicError(413, 'payload_too_large')) : Promise.resolve(req.rawBody);
  if (typeof req.body === 'string') { const buf = Buffer.from(req.body, 'utf8'); return buf.length > maxBytes ? Promise.reject(publicError(413, 'payload_too_large')) : Promise.resolve(buf); }
  if (Buffer.isBuffer(req.body)) return req.body.length > maxBytes ? Promise.reject(publicError(413, 'payload_too_large')) : Promise.resolve(req.body);
  if (req.body && typeof req.body === 'object' && typeof req.on !== 'function') return Promise.resolve(Buffer.from(new URLSearchParams(req.body).toString(), 'utf8'));
  return new Promise((resolve, reject) => {
    const chunks = []; let total = 0; let done = false;
    const timer = setTimeout(() => { if (done) return; done = true; reject(publicError(408, 'request_body_timeout')); if (typeof req.destroy === 'function') req.destroy(); }, Math.max(1000, Number(timeoutMs || 8000)));
    ...
    req.on('end', () => { if (done) return; done = true; clearTimeout(timer); resolve(Buffer.concat(chunks)); });
    req.on('error', (err) => { if (done) return; done = true; clearTimeout(timer); reject(err); });
  });
}
