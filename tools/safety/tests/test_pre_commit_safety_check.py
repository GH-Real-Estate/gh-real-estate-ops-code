...
class SafetyCheckTests(unittest.TestCase):
    ...
    def test_tracked_vendor_or_cache_file_is_rejected(self) -> None:
        rel = "node_modules/credentials.json"
        problems = safety_check.scan_tracked_skip_policy(rel, {rel})
        self.assertTrue(any("Tracked file is prohibited" in problem for problem in problems))
        self.assertEqual([], safety_check.scan_tracked_skip_policy(rel, set()))
    ...
    def test_tracked_file_enumeration_fails_closed(self) -> None:
        ...
        with mock.patch.object(safety_check.subprocess, "run", return_value=result):
            tracked, problems = safety_check.load_tracked_paths(Path("."))
