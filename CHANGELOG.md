...
- Removed the false defaults and nested writes. Late-fee, monthly-interest, and returned-payment fee invoices now preserve the source invoice's top-level `allow_partial_payments` value when Zoho returns it; generic source-ledger update fallbacks carry the same value forward.
