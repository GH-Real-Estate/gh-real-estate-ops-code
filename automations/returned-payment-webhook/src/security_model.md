...
This is intentional. Do not replace the inbound HMAC model with OAuth2 unless Zoho Payments supports sending OAuth bearer tokens or custom authentication headers on webhook delivery.
...
OAuth2 credentials are stored in Catalyst Environment Variables:
...
The function includes fallback parsing for descriptions like:

```text
Zoho :: GH Real Estate :: INV-000142
```
...
## Why This Does Not Use Zoho Creator

The Sylvara webhook architecture routed through Zoho Creator because Creator was the application layer for that workflow.

This GH Real Estate webhook creates financial records. Catalyst should own the transaction because it can verify the payment, enforce replay/idempotency, call Zoho Books, and create the fee invoice in one backend process.

Routing through Creator would add moving parts without improving security.

## Enterprise Security Position

This function is enterprise-grade when the following controls are enabled:
