...
- Live IDs existed in the public base history, so this PR's alias mapping is reconstructible. Treat both IDs and aliases as public, non-secret identifiers; no claim is made that redaction erased public history or PR diffs.
- Keep any private runtime mapping outside GitHub and never use record-ID secrecy as an authorization control.
- Regenerated manifest hashes for every changed public artifact and made current/release manifests self-contained.
