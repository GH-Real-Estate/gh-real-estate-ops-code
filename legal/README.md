...
The scheduled monitor compares approved source hashes with official URLs and opens a `LEGAL REVIEW REQUIRED` issue when a source changes or cannot be verified. It never silently promotes changed text, rewrites effective dates, or merges a legal update. After human review, regenerate the complete release, update the registry and changelog, replace all eight current PDFs and their text extracts together, and record the review date.
