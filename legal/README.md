...
1. `Authority Discovery` runs weekly on Sunday at `11:17 UTC` (and on manual dispatch) against 14 configured legal official-source indexes. It looks for later publications and index changes that fixed approved URLs cannot discover. The catalog includes the Kansas Legislature's stable current Chapter 58 Article 25 index, bounded to 90–150 section links and treated as review-only metadata. Its result is `current`, `review_required`, or `degraded`; these are discovery-run states, not legal conclusions.
